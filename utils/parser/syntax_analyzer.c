...
/*   Updated: 2023/05/04 16:31:04 by abahsine         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_syntax_after_pipe(t_tokens *token)
{
	...
	|| (token->next && token->next->type == PIPE) || (token->next && token->next->type == T_SPACE
...
int	check_redirections_syntax(t_tokens *token)
{
	while (token)
	{
		...
		{
			if (!token->next || token->next->type == OR_OPERATOR || token->next->type == IR_OPERATOR || token->next->type == HEREDOC_OPERATOR || token->next->type == APPEND_OPERATOR)
